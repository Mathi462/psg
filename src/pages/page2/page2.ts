...
export class Page2Page {
  ...
  constructor(public popoverCtrl: PopoverController,public navCtrl: NavController,public storage:Storage,public navParams: NavParams, public toastCtrl: ToastController, public formBuilder: FormBuilder, private camera: Camera) {
  ...
  presentPopover(myEvent) {
    let popover = this.popoverCtrl.create(WedpopPage);
    popover.present({
      ev: myEvent
    });
    popover.onDidDismiss(() => {
      this. storage.get("wed").then(data => {
        console.log(data)
        this.Birth = data

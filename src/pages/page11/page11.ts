...
import { WedpopPage } from '../wedpop/wedpop';
import { DestpopPage } from '../destpop/destpop';
import { DeppopPage } from '../deppop/deppop';
...
export class Page11Page {
  ...
  timeD: any
  response: any;
  constructor(public popoverCtrl: PopoverController,public navCtrl: NavController, public storage: Storage, public navParams: NavParams, public toastCtrl: ToastController, public formBuilder: FormBuilder, public apiProvider: ApiProvider) {
  ...
  presentPopover(myEvent) {
    let popover = this.popoverCtrl.create(DestpopPage);
    popover.present({
      ev: myEvent
    });
    popover.onDidDismiss(() => {
      this. storage.get("dest").then(data => {
        console.log(data)
        this.dateDA = data
        console.log(this.dateDA)
    })
    })
  }
  presentPopove(myEvent) {
    let popover = this.popoverCtrl.create(DeppopPage);
    popover.present({
      ev: myEvent
    });
    popover.onDidDismiss(() => {
      this. storage.get("dep").then(data => {
        console.log(data)
        this.dateD = data
...
//   }
  nextData : any;
  next() {
    this. storage.get("dep").then(depdata => {
      console.log(depdata)
      this.dateD =depdata
      console.log(this.dateD)
 
  this. storage.get("dest").then(destdata => {
    console.log(depdata)
    this.dateDA = destdata
    console.log(this.dateDA)
